...
export function Settings() {
    ...
    const detect = async () => {
      try {
        ...
        if (creator) {
          ...
          await fetchCreatorStats(creator.id); // <-- NEW: fetch stats after profile
          setLoading(false);
          return;
        }
  ...
  const fetchCreatorStats = async (cid: string) => {
    try {
      // 1. Total completed streams across all campaigns where the creator is active/completed
      const { data: campaignsData, error: campaignsError } = await supabase
        .from("campaign_creators")
        .select("streams_completed, status")
        .eq("creator_id", cid)
        .in("status", ["active", "completed"]); // only count completed work

      if (campaignsError) throw campaignsError;
      const totalStreams = (campaignsData || []).reduce((sum, cc) => sum + (cc.streams_completed || 0), 0);

      // 2. Average viewers – if you have a viewer_count column in stream_proofs, you can average that.
      //    For now we keep the value from the profile (or 0). Update this when you have that data.
