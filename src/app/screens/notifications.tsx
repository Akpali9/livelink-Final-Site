...
export function Notifications() {
  ...
  const handleClick = (n: Notification) => {
    ...
    if (isBiz) {
      ...
      if (["campaign", "campaign_approved", "campaign_rejected"].includes(n.type))
        return navigate(`/business/campaigns`);
      if (["payment", "payout", "earnings"].includes(n.type))
        return navigate(`/business/dashboard`);
      if (["new_offer", "offer"].includes(n.type))
        return navigate(`/business/campaigns`);
      return navigate(`/business/dashboard`);
    } else {
      ...
      if (["earnings", "payment", "payout"].includes(n.type))
        return navigate(`/dashboard`);
      if (["campaign", "offer", "new_offer"].includes(n.type))
    ...
    return (
      <div className="flex flex-col min-h-screen bg-white text-[#1D1D1D] max-w-[480px] mx-auto w-full">
        <AppHeader
          ...
          title="Notifications"
          backPath={backPath}
          userType={isBiz ? "business" : "creator"}
        />
