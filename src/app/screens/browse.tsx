      ...
      try {
        ...
        if (campaignsData) {
          const formattedCampaigns = campaignsData.map(c => ({
            // Explicitly pick only known scalar fields — never spread raw DB rows
            id: c.id,
            business_id: c.business_id,
            created_at: c.created_at,
            closing_date: c.end_date || c.closing_date,
            is_featured: c.is_featured || false,
            budget: c.budget,
            streams_required: c.streams_required || 4,
            min_viewers: c.min_viewers || 0,
            budget_range: typeof c.budget_range === 'string' ? c.budget_range : '₦50k-₦200k',
            name: typeof c.name === 'string' ? c.name : '',
            industry: typeof c.business?.industry === 'string' ? c.business.industry : (typeof c.industry === 'string' ? c.industry : ''),
            logo: typeof c.business?.logo_url === 'string' ? c.business.logo_url : 'https://via.placeholder.com/100',
            partnership_type: (typeof c.type === 'string' ? c.type : '') as PartnershipType,
            pay_rate: c.budget ? `₦${c.budget}` : 'Negotiable',
            location: typeof c.business?.city === 'string' ? c.business.city : 'Remote',
            description: typeof c.description === 'string' ? c.description : (typeof c.business?.description === 'string' ? c.business.description : ''),
            about: typeof c.about === 'string' ? c.about : (typeof c.description === 'string' ? c.description : ''),
            response_rate: '95%',
            is_verified: c.business?.id_verified || false,
            // Sanitize niche_tags: ensure always string[]
            niche_tags: Array.isArray(c.niche_tags)
              ? c.niche_tags.filter((t: any) => typeof t === 'string')
              : [],
            // Safe business object — only scalar fields we actually render
            business: c.business ? {
              id: c.business.id,
              business_name: typeof c.business.business_name === 'string' ? c.business.business_name : '',
              logo_url: typeof c.business.logo_url === 'string' ? c.business.logo_url : '',
              industry: typeof c.business.industry === 'string' ? c.business.industry : '',
              location: typeof c.business.city === 'string' ? c.business.city : '',
              description: typeof c.business.description === 'string' ? c.business.description : '',
              is_verified: c.business.id_verified || false,
            } : undefined,
          }));
    ...
    async function fetchUserData() {
      try {
        ...
        const { data: profileData } = await supabase
          ...
          .select("id, user_id, avg_concurrent, full_name, niches, avatar_url, rating")
          .eq("user_id", user.id)
          .single();
