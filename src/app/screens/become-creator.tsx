...
export function BecomeCreator() {
  ...
  const onSubmit = async (data: CreatorFormData) => {
    ...
    try {
      ...
      // 2. Upload verification doc if provided
      ...
      // 3. Insert creator_profiles row
      // ✅ All columns now exist after running creator_profiles_extension.sql
      // ✅ status: 'pending_review' is valid after extending the CHECK constraint
      // ✅ niche is the correct schema column name (mapped from form's "categories")
      // ✅ avg_viewers is the correct schema column name (mapped from form's "avgConcurrent")
      // ✅ location is set to "city, country" since schema has a single location TEXT field
      const { data: profileData, error: profileError } = await supabase
        .from("creator_profiles")
        .insert({
          user_id:                    authData.user.id,
          full_name:                  data.fullName,           // ✅ schema: full_name
          email:                      data.email.toLowerCase().trim(), // ✅ added by migration
          dob:                        data.dob,                // ✅ added by migration
          age:                        calculateAge(),          // ✅ added by migration
          phone_number:               data.phoneNumber,        // ✅ added by migration
          country:                    data.country,            // ✅ added by migration
          city:                       data.city,               // ✅ added by migration
          location:                   `${data.city}, ${data.country}`, // ✅ existing schema col
          niche:                      data.categories || [],   // ✅ schema: niche (not "categories")
          avg_viewers:                data.avgConcurrent ? parseInt(data.avgConcurrent) : 0, // ✅ schema: avg_viewers (not avg_concurrent)
          total_streams:              0,
          streaming_frequency:        data.frequency,          // ✅ added by migration
          streaming_duration:         data.duration,           // ✅ added by migration
          streaming_days:             data.days || [],         // ✅ added by migration
          streaming_time:             data.timeOfDay,          // ✅ added by migration
          avg_peak:                   data.avgPeak ? parseInt(data.avgPeak) : 0,     // ✅ added by migration
          avg_weekly:                 data.avgWeekly ? parseInt(data.avgWeekly) : 0, // ✅ added by migration
          audience_bio:               data.audienceBio || "",  // ✅ added by migration
          referral_code:              data.referral || null,   // ✅ added by migration
          verification_document_url:  verificationUrl,         // ✅ added by migration
          status:                     "pending_review",        // ✅ valid after CHECK constraint update
        })
        .select("id")
        .single();
      ...
      // 4. Insert platforms into creator_platforms (separate table)
      // ✅ platforms are NOT a JSONB column on creator_profiles — they are a separate table
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <input
                      ...
                      className={`w-full bg-[#F8F8F8] border p-5 text-sm font-bold uppercase tracking-tight outline-none focus:border-[#1D1D1D] transition-all italic ${errors.fullName ? "border-red-500" : "border-[#1D1D1D]/10"}`}
                    />
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full bg-[#F8F8F8] border p-5 pl-12 text-sm font-bold uppercase tracking-tight outline-none focus:border-[#1D1D1D] transition-all ${errors.dob ? "border-red-500" : "border-[#1D1D1D]/10"}`}
                      />
                    </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full bg-[#F8F8F8] border p-5 pl-12 text-sm font-bold uppercase tracking-tight outline-none focus:border-[#1D1D1D] transition-all italic ${errors.email ? "border-red-500" : "border-[#1D1D1D]/10"}`}
                      />
                    </div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          {...register("password", {
                            ...
                            pattern: { value: /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d@$!%*#?&]{6,}$/, message: "Must contain a letter and a number" },
                          })}
                          className={`w-full bg-[#F8F8F8] border p-5 text-sm font-bold uppercase tracking-tight outline-none focus:border-[#1D1D1D] transition-all ${errors.password ? "border-red-500" : "border-[#1D1D1D]/10"}`}
                        />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2">
                          {showPassword ? <EyeOff className="w-4 h-4 opacity-30 hover:opacity-100" /> : <Eye className="w-4 h-4 opacity-30 hover:opacity-100" />}
                        </button>
                      </div>
                      {getPasswordStrength() && (
                        <div className="mt-1">
                          <div className="flex gap-1 h-1 mb-1">
                            {[1, 2, 3, 4].map((i) => (
                              <div key={i} className={`flex-1 h-full rounded-full transition-all ${i <= (getPasswordStrength()?.score || 0) ? getPasswordStrength()?.color.replace("text", "bg") : "bg-gray-200"}`} />
                            ))}
                          </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className={`w-full bg-[#F8F8F8] border p-5 text-sm font-bold uppercase tracking-tight outline-none focus:border-[#1D1D1D] transition-all ${errors.confirmPassword ? "border-red-500" : "border-[#1D1D1D]/10"}`}
                        />
                        <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-4 top-1/2 -translate-y-1/2">
                          {showConfirmPassword ? <EyeOff className="w-4 h-4 opacity-30 hover:opacity-100" /> : <Eye className="w-4 h-4 opacity-30 hover:opacity-100" />}
                        </button>
                      </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="relative flex">
                      <select className="bg-white border border-[#1D1D1D]/10 p-5 text-xs font-black uppercase tracking-tight outline-none border-r-0">
                      ...
                      <div className="relative flex-1">
                        ...
                        <input
                          type="tel"
                          {...register("phoneNumber", { required: "Phone number is required", pattern: { value: /^[0-9]{7,15}$/, message: "Invalid phone number" } })}
                          placeholder="Phone number"
                          className={`w-full bg-[#F8F8F8] border p-5 pl-12 text-sm font-bold uppercase tracking-tight outline-none focus:border-[#1D1D1D] transition-all ${errors.phoneNumber ? "border-red-500" : "border-[#1D1D1D]/10"}`}
                        />
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <select
                        ...
                        className={`w-full bg-white border p-5 text-xs font-black uppercase tracking-tight outline-none ${errors.country ? "border-red-500" : "border-[#1D1D1D]/10"}`}
                      >
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <input
                        ...
                        className={`w-full bg-[#F8F8F8] border p-5 text-sm font-bold uppercase tracking-tight outline-none focus:border-[#1D1D1D] transition-all italic ${errors.city ? "border-red-500" : "border-[#1D1D1D]/10"}`}
                      />
          ...
          {step === 2 && (
            ...
            >
              <section>
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest opacity-40 mb-8 italic">
                  Tell us where you go live. Platforms are saved separately so we can match you with relevant campaigns.
                </p>
                <div className="flex flex-col gap-8">
                  {fields.map((field, index) => (
                    <motion.div
                      ...
                      className="relative p-8 border-2 border-[#1D1D1D] bg-white"
                    >
                      ...
                      <div className="flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input
                            {...register(`platforms.${index}.username` as const, { required: "Username required" })}
                            placeholder="e.g. @creatorname"
                            className="w-full bg-[#F8F8F8] border border-[#1D1D1D]/10 p-5 text-sm font-bold uppercase tracking-tight outline-none focus:border-[#1D1D1D] italic"
                          />
                        </div>
                        <div className="flex flex-col gap-1.5">
                  ...
                  {fields.length < 5 && (
                    <button
                      ...
                      className="w-full border-2 border-dashed border-[#1D1D1D]/20 p-8 flex flex-col items-center gap-2 hover:border-[#1D1D1D] transition-all text-[#1D1D1D]/40 hover:text-[#1D1D1D] group"
                    >
            ...
            >
              <section>
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest opacity-40 mb-12 italic">
                  Be as accurate as possible — this determines which campaigns you match with.
                </p>
                <div className="flex flex-col gap-12">
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-3">
                      {[
                        ...
                        { val: "Monthly or less", sub: "Once a month or occasionally" },
                      ].map((opt) => (
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <div className="bg-[#FEDB71]/10 border-2 border-[#FEDB71] p-4 text-[9px] font-black uppercase tracking-widest text-center italic">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => (
                          <label key={day} className="cursor-pointer">
                            ...
                            <div className="w-12 h-12 flex items-center justify-center border-2 border-[#1D1D1D]/10 bg-white peer-checked:bg-[#389C9A] peer-checked:text-white peer-checked:border-[#389C9A] text-[10px] font-black transition-all italic">
                              {day[0]}
                            </div>
                          </label>
                        ))}
                  ...
                  <div className="flex flex-col gap-8">
                    ...
                    {[
                      { field: "avgConcurrent" as const, label: "Average concurrent viewers per stream", placeholder: "e.g. 250" },
                      { field: "avgPeak" as const, label: "Average peak viewers per stream", placeholder: "e.g. 500" },
                      { field: "avgWeekly" as const, label: "Average weekly total viewers", placeholder: "e.g. 1200" },
                    ].map((item, i) => (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {["Gaming", "Beauty & Makeup", "Fashion", "Fitness & Health", "Food & Cooking", "Music", "Comedy", "Education", "Business & Finance", "Lifestyle", "Sports", "Tech", "Travel", "Other"].map((cat) => (
                        <label key={cat} className="cursor-pointer">
                          ...
                          <div className="px-4 py-2 border-2 border-[#1D1D1D]/10 bg-white peer-checked:bg-[#389C9A] peer-checked:text-white peer-checked:border-[#389C9A] text-[9px] font-black uppercase tracking-widest transition-all italic">
                            {cat}
                          </div>
                        </label>
                      ))}
          ...
          {step === 4 && (
            ...
            >
              <section>
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest opacity-40 mb-8 italic">
                  Proof of your streaming analytics from the last 30 days. Stored in a private bucket.
                </p>
                <div className="relative">
                  <input
                    type="file"
                  ...
                  <label
                    ...
                    className="border-2 border-dashed border-[#1D1D1D]/20 p-12 flex flex-col items-center gap-6 bg-[#F8F8F8] group hover:border-[#1D1D1D] transition-all cursor-pointer"
                  >
              ...
              <section>
                ...
                <div className="bg-[#F8F8F8] border-2 border-[#1D1D1D] p-8 flex flex-col gap-6">
                  {[
                    ...
                    { label: "Main Platform", value: platforms?.[0]?.type },
                    { label: "Stream Frequency", value: frequency },
                    { label: "Verification Doc", value: selectedFile ? selectedFile.name : "Not uploaded (optional)" },
                  ].map((row, i, arr) => (
        ...
        <div className="flex gap-4">
          {step > 1 && (
            <button
              onClick={prevStep}
              disabled={isLoading}
              className="px-6 py-5 border-2 border-[#1D1D1D] font-black uppercase tracking-widest text-[10px] hover:bg-[#F8F8F8] transition-all italic disabled:opacity-50"
            >
              Back
            </button>
          )}
