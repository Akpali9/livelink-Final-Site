      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `receiver_id=eq.${user.id}` },
        async (payload) => {
          ...
          toast.info(`New message from ${senderName}`, {
            ...
            icon: '💬',
            action: { label: 'Reply', onClick: () => navigate(`/messages/${payload.new.sender_id}`) }
          });
        }
      )
    ...
    <header className="px-5 pt-10 pb-4 border-b border-[#1D1D1D]/10 sticky top-0 bg-white z-50">
      <div className="flex justify-between items-center max-w-7xl mx-auto">
        <div className="flex items-center gap-3">
          {showBack && (
            <button onClick={() => backPath ? navigate(backPath) : navigate(-1)} className="p-1 -ml-1 active:bg-[#1D1D1D]/10 transition-colors rounded-none" aria-label="Go back">
              <ArrowLeft className="w-5 h-5" />
            </button>
          )}
          {showHome && (
            <button onClick={() => navigate(dashboardPath)} className="p-1 -ml-1 active:bg-[#1D1D1D]/10 transition-colors rounded-none" aria-label="Go to dashboard">
              <Home className="w-5 h-5" />
            </button>
          )}
          {showLogo && (
            <div className="flex flex-col cursor-pointer" onClick={() => isAuthenticated ? navigate(dashboardPath) : navigate('/')}>
        ...
        <div className="flex items-center gap-3 relative">
            ...
            <>
              ...
              <div className="relative">
                    ...
                    <>
                      ...
                      >
                        <div className="p-4 border-b-2 border-[#1D1D1D] bg-[#F8F8F8] flex justify-between items-center sticky top-0 z-10">
                          ...
                          {unreadNotifications > 0 && (
                            <button onClick={markAllNotificationsAsRead} className="text-[8px] font-black uppercase tracking-widest text-[#389C9A] hover:underline">Mark All Read</button>
                          )}
                        </div>
                        <div className="overflow-y-auto" style={{ maxHeight: '400px' }}>
                          {notifications.length === 0 ? (
                            <div className="p-8 text-center">
                              <div className="w-12 h-12 bg-[#F8F8F8] mx-auto mb-3 flex items-center justify-center border border-[#1D1D1D]/10"><Bell className="w-5 h-5 opacity-20" /></div>
                            ...
                            <div key={notif.id} onClick={() => markNotificationAsRead(notif.id)} className={`p-4 border-b border-[#1D1D1D]/10 hover:bg-[#F8F8F8] transition-colors cursor-pointer ${!notif.is_read ? 'bg-[#FEDB71]/5' : ''}`}>
                        ...
                        {notifications.length > 0 && (
                          <div className="p-3 border-t-2 border-[#1D1D1D] bg-[#F8F8F8] sticky bottom-0">
                            <Link to={notificationsPath} onClick={() => setShowNotifications(false)} className="block text-center text-[9px] font-black uppercase tracking-widest text-[#389C9A] hover:underline">View All Notifications →</Link>
                          </div>
                        )}
                      </motion.div>
                    </>
                ...
                <AnimatePresence>
                  {showMessages && (
                    <>
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40" onClick={() => setShowMessages(false)} />
                      <motion.div
                        initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ duration: 0.15 }}
                        className="absolute right-0 mt-2 w-80 sm:w-96 bg-white border-2 border-[#1D1D1D] shadow-xl z-50 max-w-[calc(100vw-2rem)]"
                        style={{ maxHeight: 'calc(100vh - 100px)', overflowY: 'auto' }}
                      >
                        <div className="p-4 border-b-2 border-[#1D1D1D] bg-[#F8F8F8] flex justify-between items-center sticky top-0 z-10">
                          ...
                          {unreadMessages > 0 && (
                            <span className="bg-[#389C9A] text-white text-[8px] font-black px-2 py-1">{unreadMessages} unread</span>
                          )}
                        </div>
                        <div className="overflow-y-auto" style={{ maxHeight: '400px' }}>
                          {recentMessages.length === 0 ? (
                            ...
                            <Link key={msg.id} to={`/messages/${msg.sender_id}`} onClick={() => { setShowMessages(false); if (!msg.is_read) markMessageAsRead(msg.id); }} className={`block p-4 border-b border-[#1D1D1D]/10 hover:bg-[#F8F8F8] transition-colors ${!msg.is_read ? 'bg-[#389C9A]/5' : ''}`}>
                              <div className="flex items-start gap-3">
                                {msg.sender_avatar ? (
                                  <ImageWithFallback src={msg.sender_avatar} className="w-10 h-10 border-2 border-[#1D1D1D]/10 object-cover flex-shrink-0" />
                                ) : (
                                  <div className="w-10 h-10 bg-[#1D1D1D]/5 border-2 border-[#1D1D1D]/10 flex items-center justify-center flex-shrink-0"><User className="w-5 h-5 opacity-40" /></div>
                                )}
                                <div className="flex-1 min-w-0">
                                  <div className="flex justify-between items-start mb-1">
                                    <p className="text-[11px] font-black uppercase tracking-widest truncate">{msg.sender_name}</p>
                                    <span className="text-[8px] opacity-40 whitespace-nowrap ml-2 flex-shrink-0">{formatTimestamp(msg.created_at)}</span>
                                  </div>
                                  <p className="text-[9px] opacity-60 line-clamp-2 break-words">{msg.content}</p>
                                </div>
                                {!msg.is_read && <div className="w-2 h-2 bg-[#389C9A] flex-shrink-0 mt-2" />}
                              </div>
                            </Link>
                          ))}
                        </div>
                        {recentMessages.length > 0 && (
                          <div className="p-3 border-t-2 border-[#1D1D1D] bg-[#F8F8F8] sticky bottom-0">
                            <Link to={messagesPath} onClick={() => setShowMessages(false)} className="block text-center text-[9px] font-black uppercase tracking-widest text-[#389C9A] hover:underline">View All Messages →</Link>
                          </div>
                        )}
                      </motion.div>
                    </>
          ...
          <div className="relative ml-1">
            <button
              onClick={() => {
                if (isAuthenticated) { setShowProfileMenu(!showProfileMenu); setShowNotifications(false); setShowMessages(false); }
                else navigate('/login/portal');
              }}
            ...
            <AnimatePresence>
              {showProfileMenu && isAuthenticated && (
                <>
                  ...
                  <motion.div
                    initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} transition={{ duration: 0.1 }}
                  ...
                  >
                    <div className="p-4 border-b-2 border-[#1D1D1D] bg-[#F8F8F8]">
                      <div className="flex items-center gap-3 mb-3">
                        {userAvatar
                          ...
                          : <div className="w-10 h-10 bg-[#1D1D1D]/5 border-2 border-[#1D1D1D]/10 flex items-center justify-center"><User className="w-5 h-5 opacity-40" /></div>
                        }
                        <div className="flex-1 min-w-0">
                    ...
                    <div className="p-2">
                      ...
                      {isBusiness ? (
                        ...
                        <Link to="/dashboard" onClick={() => setShowProfileMenu(false)} className="w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-[#1D1D1D] hover:text-white flex items-center gap-3 transition-colors rounded-none">
                          <Home className="w-3.5 h-3.5 text-[#389C9A]" /> Dashboard
                        </Link>
                      )}
                      <button onClick={handleLogout} className="w-full text-left px-4 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-[#1D1D1D] hover:text-white text-red-500 flex items-center gap-3 transition-colors rounded-none">
